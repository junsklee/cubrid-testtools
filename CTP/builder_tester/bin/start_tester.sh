...
LOG_FILE="$SCRIPT_DIR/tester_output.log"
...
if [ -f "$PID_FILE" ]; then
  ...
  if [ -n "${oldpid:-}" ] && kill -0 "$oldpid" 2>/dev/null; then
    echo "Tester already running with pid $oldpid (PID file: $PID_FILE)" >&2
    exit 0
  fi
fi
...
echo "$PID" > "$PID_FILE"
echo "Tester started (pid $PID). Logs: $LOG_FILE, PID file: $PID_FILE"
