...
private Optional<Assignment> assignTest(TestInstance test, String category) {
    ...
    // Ramp-up deferral (optional, configurable): Defer long + IO-heavy tests
    // based on actual I/O utilization to prevent I/O bottlenecks during initial load
    if (config.useRampUpDeferral() && isRampDeferred(test)) {
        eligible = eligible.stream()
            .filter(n -> hasIoHeadroomForHeavyTest(n, test))
            .collect(Collectors.toList());
        if (eligible.isEmpty()) {
            logger.info("[Ramp-Up Deferral] Test deferred - no nodes with sufficient I/O headroom");
            return Optional.empty();
        }
    }
...
**Ramp-Up Deferral (Optional):**

Resource-based ramp-up deferral prevents I/O bottlenecks during initial test batch loading by deferring heavy I/O tests until the cluster has sufficient capacity headroom.

**Problem Solved:**

When a large batch of tests arrives, if many long-duration + high-I/O tests ("elephants") start simultaneously, they can saturate disk I/O and cause:
- Disk contention and reduced throughput
- Extended test runtimes due to I/O starvation
- Cascading delays across the entire batch

The original count-based ramp-up (`running >= max_concurrent/2`) could deadlock when most tests were elephants—short tests ("mice") would finish before reaching the threshold, permanently blocking elephants.

**How It Works:**

Ramp-up deferral applies to tests classified as "heavy" (duration ≥60s **AND** total I/O ≥60 MB/s). During assignment, eligible nodes are filtered through three I/O capacity checks:

1. **Current Utilization Check:**
   - Calculates current I/O usage as percentage of total capacity: `(usedRead + usedWrite) / (capacityRead + capacityWrite)`
   - Defers test if current utilization ≥ threshold (default: 40%)
   - Prevents adding load to already-stressed nodes

2. **Projected Utilization Check:**
   - Calculates utilization after adding this test's I/O demand
   - Defers test if projected utilization ≥ threshold
   - Prevents accepting tests that would push node over threshold

3. **Absolute Headroom Check:**
   - Calculates free I/O bandwidth in MB/s: `totalCapacity - totalUsedIo`
   - Defers test if free I/O < minimum headroom (default: 80 MB/s)
   - Ensures minimum safety margin regardless of utilization percentage

**Algorithm:**

```java
private boolean hasIoHeadroomForHeavyTest(NodeSnapshot node, TestInstance test) {
    // Collect current I/O metrics
    double usedIo = node.getUsedIoReadMbPerSec() + node.getUsedIoWriteMbPerSec();
    double capacity = (node.getIoReadCapacityBytesPerSec() + node.getIoWriteCapacityBytesPerSec())
                      / (1024.0 * 1024.0);

    // Graceful degradation: bypass if capacity unknown
    if (capacity <= 0) {
        logger.fine("Node reports zero I/O capacity, bypassing ramp-up deferral");
        return true;
    }

    // Get config thresholds
    double threshold = config.getRampUpIoUtilizationThreshold();  // default: 0.40
    double minHeadroom = config.getRampUpMinIoHeadroomMbps();     // default: 80.0

    // Check 1: Current utilization
    double currentUtil = usedIo / capacity;
    if (currentUtil >= threshold) {
        logger.info(String.format("[Ramp-Up] Node %s I/O utilization %.1f%% >= threshold %.1f%%",
                    node.getNodeId(), currentUtil * 100, threshold * 100));
        return false;
    }

    // Check 2: Projected utilization
    double testIo = test.getPredictedIoReadMbPerSec() + test.getPredictedIoWriteMbPerSec();
    double projectedUtil = (usedIo + testIo) / capacity;
    if (projectedUtil >= threshold) {
        logger.info(String.format("[Ramp-Up] Node %s projected I/O %.1f%% >= threshold %.1f%%",
                    node.getNodeId(), projectedUtil * 100, threshold * 100));
        return false;
    }

    // Check 3: Absolute headroom
    double freeIo = capacity - usedIo;
    if (freeIo < minHeadroom) {
        logger.info(String.format("[Ramp-Up] Node %s free I/O %.1f MB/s < minimum %.1f MB/s",
                    node.getNodeId(), freeIo, minHeadroom));
        return false;
    }
...
If **all** nodes fail these checks for a heavy test, the test is deferred and will be retried in the next scheduling cycle. Light tests and short tests bypass these checks entirely.

**Graceful Degradation:**

If a node reports zero or unknown I/O capacity, the ramp-up checks are bypassed for that node, preventing scheduling deadlocks in environments where I/O metrics are unavailable.
...
# Enable/disable ramp-up deferral (disabled by default for backward compatibility)
use_ramp_up_deferral=false

# Maximum I/O utilization percentage before deferring heavy tests (0.0-1.0)
# Lower = more conservative, higher = more aggressive scheduling
# Recommended: 0.30-0.50 depending on disk speed and test mix
ramp_up_io_utilization_threshold=0.40

# Minimum free I/O bandwidth (MB/s) required to accept heavy tests
# Should be set based on typical heavy test I/O demand
# Recommended: 1-2x average heavy test I/O bandwidth
ramp_up_min_io_headroom_mbps=80.0
```

**When to Enable:**

- **Enable** (`use_ramp_up_deferral=true`) when:
  - I/O capacity is limited or shared across multiple workloads
  - Test batches contain many long-duration, I/O-intensive tests
  - Disk saturation has been observed during initial test loading
  - Historical data shows I/O contention reduces throughput

- **Disable** (`use_ramp_up_deferral=false`) when:
  - I/O capacity is abundant relative to test demands
  - Test batches are mostly short or I/O-light
  - Maximizing parallel execution is more important than I/O smoothing
  - I/O metrics are unreliable or unavailable

**Logging:**

When ramp-up deferral blocks a test assignment, an INFO-level log is written:

```
[Ramp-Up Deferral] Test 'sql/_27_banana_qa/issue_1234' deferred - no nodes with sufficient I/O headroom (3 nodes filtered out due to I/O capacity limits)
```

Detailed per-node deferral reasons are logged when checks fail:

```
[Ramp-Up] Node tester-1 I/O utilization 45.2% >= threshold 40.0%, deferring test 'sql/_27_banana_qa/issue_1234'
[Ramp-Up] Node tester-2 projected I/O utilization 52.8% >= threshold 40.0%, deferring test 'sql/_27_banana_qa/issue_1234' (test needs 85.3 MB/s, current usage 125.6 MB/s)
[Ramp-Up] Node tester-3 free I/O 65.4 MB/s < minimum 80.0 MB/s, deferring test 'sql/_27_banana_qa/issue_1234'
```
...
private boolean isRampDeferred(TestInstance test) {
    // A test is subject to ramp-up deferral if it meets BOTH criteria:
    // 1. Long duration (≥60 seconds)
    // 2. High I/O (≥60 MB/s combined read+write)

    double durationSec = test.getPredictedDurationSec();
    double ioMbPerSec = test.getPredictedIoReadMbPerSec() + test.getPredictedIoWriteMbPerSec();

    return durationSec >= 60.0 && ioMbPerSec >= 60.0;
}
```

**Implementation:**

- Test classification: `SchedulerService.isRampDeferred()` in [SchedulerService.java](../src/com/navercorp/cubridqa/builder/scheduler/SchedulerService.java)
- Three-check algorithm: `SchedulerService.hasIoHeadroomForHeavyTest()` in [SchedulerService.java:268-348](../src/com/navercorp/cubridqa/builder/scheduler/SchedulerService.java#L268-L348)
- Configuration getters: `BuilderConfig.useRampUpDeferral()`, `getRampUpIoUtilizationThreshold()`, `getRampUpMinIoHeadroomMbps()` in [BuilderConfig.java](../src/com/navercorp/cubridqa/builder/BuilderConfig.java)
...
- **Resource-based ramp-up guard** (optional): Long + IO-heavy jobs (duration ≥60s AND I/O ≥60 MB/s) are deferred based on three I/O capacity checks: (1) current utilization ≥ threshold, (2) projected utilization ≥ threshold, (3) free I/O < minimum headroom. Prevents disk saturation during initial batch loading while avoiding the deadlock issues of count-based ramp-up. Configured via `use_ramp_up_deferral` (default: false), `ramp_up_io_utilization_threshold` (default: 0.40), and `ramp_up_min_io_headroom_mbps` (default: 80.0). Includes graceful degradation when I/O capacity metrics are unavailable. Recommended for environments with limited I/O capacity or heavy I/O test mixes.
